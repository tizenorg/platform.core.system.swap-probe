...
int fcntl(int fd, int cmd, ...)
{
	...
	if (api_type == FD_API_LOCK_START) {
		AFTER_PACK_ORIGINAL_FD(API_ID_fcntl,
				       ...
				       "ddddxx", fd, cmd, type,
				       whence, start, len);
		PRINTERR("LOCK_START blockresult=%d", blockresult);
	}
	...
	if (api_type != FD_API_OTHER) {
		...
		AFTER_PACK_ORIGINAL_FD(API_ID_fcntl,
				       ...
				       "ddddxx", fd, cmd, type,
				       whence, start, len);
		PRINTERR("LOCK_END/UNLOCK %d blockresult=%d", api_type, blockresult);
	} else {
		/* pack FD_API_OTHER */
		AFTER_PACK_ORIGINAL_FD(API_ID_fcntl,
				       'd', ret, 0, fd, api_type,
				       "ddd", fd, cmd, arg);
		PRINTERR("FD_API_OTHER %d", blockresult);
	}
