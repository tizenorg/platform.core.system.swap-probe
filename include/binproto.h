...
#define HANDLER_DEF(_ret, _name, ...)         \
	static _ret _name##_handler(uint32_t call_type, uint64_t caller, __VA_ARGS__)
...
{                                                                           \
    ...
    return _name##_handler(INTERNAL_CALL, caller  FUNC_CALL(__VA_ARGS__);   \
}                                                                           \
                                                                            \
...
{                                                                           \
    ...
    caller = (uint64_t)                                                     \
        (__builtin_extract_return_addr(__builtin_return_address(0)));       \
    return _name##_handler(EXTERNAL_CALL, caller  FUNC_CALL(__VA_ARGS__);   \
}


#define HANDLER_DEF_THROW(_ret, _name, _throw, ...)         \
	static _ret _name##_handler(uint32_t call_type, uint64_t caller, __VA_ARGS__) throw _throw
