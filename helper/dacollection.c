		   ...
		   struct __elm_gesture_layer_cb_set_data *elm2)
{
	...
	PRINTERR("update>");
	PRINTMSG("1>obj=%p idx=%d cb_type=%d cb=%p data=%p",
		 elm1->obj,
		 elm1->idx,
		 elm1->cb_type,
		 elm1->cb,
	...
	PRINTMSG("2>obj=%p idx=%d cb_type=%d cb=%p data=%p",
		 elm2->obj,
		 elm2->idx,
		 elm2->cb_type,
		 elm2->cb,
...
void *add_gesture_hash(void *ptr)
{
	...
	if (GESTUREHASH == NULL) {
		PRINTERR("ERR_NOTINITIALIZED");
		goto exit;
	}

	if (ptr == NULL) {
		PRINTERR("ERR_WRONGPARAMETER");
		goto exit;
	}
