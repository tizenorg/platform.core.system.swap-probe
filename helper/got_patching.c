...
static void *_get_orig_on_demand(unsigned long addr)
{
	...
	if (!strcmp(probe_lib, sym_info.dli_fname))
		return NULL;
...
/* TODO REMOVE THIS. Find a proper way to handle uncatched fixups */
static void _process_ui_events(void)
{
	unsigned int i;

	for (i = 0; i < features_cnt; i++) {
		if (features[i]->feature == FL_USER_EVENT) {
			_patch_target_bins(features[i]);
			break;
		}
	}
...
//	got_addr = lmap_reladdr_by_offset(l, reloc_arg);
	...
	/* TODO REMOVE THIS. Investigate which else functions resolves plt
	   addresses and handle it */
