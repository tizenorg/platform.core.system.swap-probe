...
function generate_addres_list()
{
    ...
    while read line; do
        if [ "$line" != "" ];then
            ...
            addr=`cat /tmp/$lib.list | grep -e " $func\$" | awk '{print $2}'| sort | uniq`
            ...
            if [[ "$handl_addr" == ""  || 0x$handl_addr -eq 0x0 ]];then
                echo "WARNING: func <$func> not found in $da_tizen_file file<$filename>">&2
                continue
            fi

            if [[ "$addr" == "" || 0x$addr -eq 0x0 ]];then
                echo "WARNING: func <$func> not found in $lib. addr=$addr file<$filename>">&2
                continue
            fi
