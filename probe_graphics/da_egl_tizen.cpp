...
{
	typedef EGLBoolean (*methodType)(EGLDisplay dpy, EGLint *major, EGLint *minor);
	/* probe prepare */
...
{
	typedef EGLBoolean (*methodType)(EGLDisplay dpy, EGLConfig *configs, EGLint config_size, EGLint *num_config);
	/* probe prepare */
...
EGLBoolean eglChooseConfig(EGLDisplay dpy, const EGLint *attrib_list,
			   ...
			   EGLint *num_config)
{
	typedef EGLBoolean (*methodType)(EGLDisplay dpy, const EGLint *attrib_list,
					 EGLConfig *configs, EGLint config_size,
					 EGLint *num_config);
	/* probe prepare */
	BEFORE_EGL(eglChooseConfig);
	/* call original function */
	EGLBoolean ret = eglChooseConfigp(dpy, attrib_list, configs, config_size,
					  num_config);
	/* get error code */
	EGL_GET_ERROR();
	/* pack and send */
	AFTER('d', ret, APITYPE_CONTEXT, "",
	      ...
	      voidp_to_uint64(configs), config_size, voidp_to_uint64(num_config));
	return ret;
}
				  ...
				  const EGLint *attrib_list)
{
	...
	/* call original function */
	EGLSurface ret = eglCreatePixmapSurfacep(dpy, config, pixmap, attrib_list);
	/* get error code */
	EGL_GET_ERROR();
	/* pack and send */
	AFTER('p', voidp_to_uint64(ret), APITYPE_CONTEXT, "",
...
{
	typedef EGLAPI __eglMustCastToProperFunctionPointerType (*methodType)(const char *procname);
	/* probe prepare */
	BEFORE_EGL(eglGetProcAddress);
	/* call original function */
	__eglMustCastToProperFunctionPointerType ret = eglGetProcAddressp(procname);
	/* get error code */
	EGL_GET_ERROR();
	/* pack and send */
